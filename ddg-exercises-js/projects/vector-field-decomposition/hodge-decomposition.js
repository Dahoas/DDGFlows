...
class HodgeDecomposition {
	...
	constructor(geometry) {
		...
		this.A = this.A.plus(SparseMatrix.diag(DenseMatrix.constant(1e-8,this.A.nRows(),1)));

		// TODO: construct two form matrix
		this.B = this.d1.timesSparse(this.hodge1Inv.timesSparse(this.d1T)); // placeholder
	}
	...
	computeExactComponent(omega) {
		// TODO
		let res = this.d0T.timesDense(this.hodge1.timesDense(omega));
		let factor = this.A.chol();
		res = factor.solvePositiveDefinite(res);
		return this.d0.timesDense(res);
	}
	...
	computeCoExactComponent(omega) {
		// TODO
		let res = this.d1.timesDense(omega);
		let factor = this.B.lu();
		res = factor.solveSquare(res);
		//res = this.hodge2Inv.timesDense(res);
		return this.hodge1Inv.timesDense(this.d1T.timesDense(res/*this.hodge2.timesDense(res)*/));
	}
	...
	computeHarmonicComponent(omega, dAlpha, deltaBeta) {
		...
		return omega.minus(dAlpha).minus(deltaBeta);
	}
}
