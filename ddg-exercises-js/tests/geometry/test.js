			...
			for (let v of mesh.vertices) {
				...
				if (n.isValid()) {
					if (sphereInscribedNormals_sol[v.index].minus(n).norm() > 1e-5) {
						//console.log(sphereInscribedNormals_sol[v.index].minus(n).norm());
						success = false;
						break;
					}
			...
			for (let v of mesh.vertices) {
				...
				if (Math.abs(minPrincipalCurvatures_sol[v.index] - k1) > 1e-5 ||
					Math.abs(maxPrincipalCurvatures_sol[v.index] - k2) > 1e-5) {
						console.log(minPrincipalCurvatures_sol[v.index]);
						console.log(k1);
						console.log(maxPrincipalCurvatures_sol[v.index]);
						console.log(k2);
					success = false;
					break;
				}
